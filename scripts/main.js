...
const joinPadCooldownPlayers = new Set();
const leavePadCooldownPlayers = new Set();
const startPadCooldownPlayers = new Set();
const USE_ARENA_BOUNDARY = true;
const ARENA_WARNING_DISTANCE = 50;
const ARENA_TELEPORT_DISTANCE = 70;
const warnedOutOfBoundsPlayers = new Set();
...
const announcedJoinedPlayers = new Set();
/* =========================================================
   HELPERS - IS PLAYER ON JOIN, LEAVE, OR START PAD
   ========================================================= */
function isPlayerOnBlock(player, blockPos) {
    ...
    return (playerX === blockPos.x &&
        playerY === blockPos.y &&
        playerZ === blockPos.z);
}
function handleJoinLeaveStartPads() {
...
function isPlayerOnAnyJoinPad(player) {
    return (isPlayerOnBlock(player, JOIN_PAD1) ||
...
/* =========================================================
   HELPERS - MESSAGES / UI
   ========================================================= */
function sendMessageToJoinedPlayers(message) {
    const players = getJoinedPlayers();
    for (const player of players) {
        player.sendMessage(message);
    }
}
function showTitleToJoinedPlayers(title, subtitle) {
    const players = getJoinedPlayers();
    for (const player of players) {
        try {
...
/* =========================================================
   HELPERS - JOIN / LEAVE
   ========================================================= */
function hasEnoughPlayersToContinueRound() {
...
function showScoresToJoinedPlayers() {
    const players = getJoinedPlayers();
...
function resetPlayerToNormal(player) {
    try {
...
function moveJoinedPlayersToArena() {
    const players = getJoinedPlayers();
    for (const player of players) {
...
function moveJoinedPlayersToLobby() {
    const players = getJoinedPlayers();
    for (const player of players) {
        resetPlayerToNormal(player);
...
/* =========================================================
   HELPERS - IT PLAYER
   ========================================================= */
function clearCurrentItVisuals() {
    const players = getJoinedPlayers();
    for (const player of players) {
        try {
...
function applyItVisuals(player) {
    try {
...
function handleSuccessfulTag(oldIt, taggedPlayer) {
